...
class BNInception(nn.Module):
    ...
    def build_temporal_ops(self, n_segment, is_temporal_shift='0' * 12, shift_div=8, exp=None):
    ...
    def _temporal_forward_wrap(self, layer_func, index):
        ...
        if hasattr(self, 'is_temporal_shift') and self.is_temporal_shift[index] == '1':
            ...
            def wrapped_func(x, is_residual, n_segment, fold_div, exp):
                if is_residual:
                    x_shift = TemporalShift.shift(
                        x, n_segment, fold_div=fold_div, exp=exp)
                    return F.relu(x + layer_func(x_shift))
                else:
                    x = TemporalShift.shift(
                        x, n_segment, fold_div=fold_div, exp=exp)
                    return layer_func(x)
            from functools import partial
            return partial(wrapped_func, is_residual=self.residual, n_segment=self.n_segment,
                           fold_div=self.fold_div, exp=self.exp)
        else:
            return layer_func
